...
def train2(train_loader, vae, projection, model,
    ...
    for epoch in range(num_epoch):
        for low_res, high_res in train_loader:
            ...
            with torch.no_grad():
                ...
                max_noise =  min(max(int((1000*(iterations+1))/((num_train_steps/batch_size)*num_epoch)),1),1000)
            ...
            loss = loss_fn(predicted_noise, noisy_z)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            #loss = loss_fn(high_res_predicted, high_res_resized)
            ...
            cum_timestep = torch.mean(timesteps.float())
            print(f"Iteration [{iterations+1}/{(int(num_train_steps/batch_size)*num_epoch)}] | Loss: {loss.item():.4f} | Timestep {cum_timestep:.0f}")    
            ...
            if iterations % checkpoint_frequency == 0:
                ...
                torch.save(checkpoint,  f"{save_dir}/checkpoints/{low_res_size}_{high_res_size}/lastest4.pt")
                with torch.no_grad():
                    ...
                    high_res = high_res[0]
                    low_res = low_res[0]
                    save_image(noisy_z[0], f"{image_dir}/noisey_z{timesteps[0].item()}")
                    save_image(predicted_noise[0], f"{image_dir}/predicted_noise")
                    #save_image(noisey_low_res[0],image_dir + f"/{timesteps}_noisey_low_res")
                    save_image(high_res_predicted, f"{image_dir}/predicted")
